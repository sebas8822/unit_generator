...
def _find_heading_block_y(page: fitz.Page, y_hint: float, title: str) -> Optional[float]:
    ...
    Some PDFs have unreliable link destinations (y outside page) and headings that span
    multiple blocks. We therefore:
      - treat y_hint as a weak hint (fallback to full-page scan if needed)
      - allow joining adjacent blocks to match multi-line headings
      - accept substring matches in either direction
      - bias toward earlier (top-of-page) matches to avoid footers/DOI lines
    """
    blocks = _block_list(page)
    h = float(page.rect.height)

    # If y_hint is clearly invalid, don't restrict to the hint window.
    ...
    def consider_span(i: int, k: int) -> Optional[Tuple[float, float, float]]:
        """Return (score_eff, raw_score, y0) for blocks[i:i+k] joined, or None if not joinable."""
        if i + k > len(blks):
            return None
        ...
        for j in range(i + 1, i + k):
            ...
            # join only visually contiguous lines/blocks (same column-ish, small gap)
            if abs(xj0 - x0) > 80.0:
                return None
            if yj0 > prev_y1 + 25.0:
                return None
    ...
    # Candidate indices: use hint window if it yields enough blocks, otherwise scan full page.
    idxs = list(range(len(blks)))
    if use_window:
        idxs_hint = [i for i, b in enumerate(blks) if (b[1] >= y0_min and b[1] <= y0_max)]
        if len(idxs_hint) >= 3:
            idxs = idxs_hint
    ...
    if best_y is not None and best_raw >= TITLE_MATCH_RATIO:
        return best_y

    # If we used a narrow window and didn't hit threshold, fall back to a full-page scan with a
...
def _build_entries_with_paths_and_anchors(doc: fitz.Document) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], bool]:
    ...
    # Re-sort after snapping so end-anchors are computed in correct reading order.
    entries.sort(key=lambda e: (e["page_0based"], e.get("y_snapped", e["y"]), e["toc_id"]))
