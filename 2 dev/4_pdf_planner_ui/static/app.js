...
async function loadProject(courseId) {
  ...
  try {
    ...
    // If we have an LLM plan, default view to LLM, otherwise Base
    ...
    // Explicitly update Step 6 card on load
...
// ---------------------------------------------------------------------------
// ... [Existing standard helpers omitted for brevity, logic remains identical] ...
...
function renderPdfList() {
  ...
  if (entries.length === 0) { list.innerHTML = `<div class="muted small">No PDFs uploaded yet.</div>`; return; }
...
function renderSlideGenerationCard() {
  ...
  const hasPlan = !!(plan && plan.weeks);

  // --- LINKAGE FIX: Clearly show where slides go based on Step 5 ---
  if (sourceLabel) {
      if (mode === "llm") {
          ...
          if (!mod) {
              sourceLabel.innerHTML = `<span style="color:#d00; font-weight:bold;">⚠️ Select Model in Step 5</span> <span class="muted small">(to define output folder)</span>`;
              if (btnWeek) btnWeek.disabled = true;
              if (btnAll) btnAll.disabled = true;
          } else {
              // Mirror backend folder naming
              const folder = (prov + "__" + mod)
                  .replace(/[\/\\]/g, "_")
                  .replace(/[^A-Za-z0-9_.-]+/g, "_")
                  .replace(/_+/g, "_")
                  .replace(/^[._-]+|[._-]+$/g, "")
                  .slice(0, 80) || "llm__model";

              sourceLabel.innerHTML = `Folder: <code>final_presentations/${folder}/</code>`;
              sourceLabel.style.color = "#338";
              
              if (btnWeek) btnWeek.disabled = !hasPlan;
              if (btnAll) btnAll.disabled = !hasPlan;
          }
      } else {
          sourceLabel.innerHTML = `Folder: <code>final_presentations/base_content/</code>`;
  ...
  if (status && !status.textContent) {
    status.textContent = hasPlan ? "Ready." : "Generate a plan first.";
  }
}
...
function readLLMFromUI() {
  ...
  // If dropdown is active and has value, use it.
  if (modSel && modSel.value && !modSel.selectedOptions[0]?.disabled) {
       state.llm.model = modSel.value;
  } else {
       // If invalid (e.g. after provider switch), clear it so Step 6 warns user
       state.llm.model = ""; 
  }
...
function updateLLMFieldsUI() {
    ...
    // Manage Generate Buttons based on Model validity
    ...
    // If we have a model (from localStorage), inject it into dropdown if empty
    const modSel = el("llmModelSelect");
    if (modSel && state.llm.model && modSel.options.length === 0) {
        // Create a temporary option so UI matches state
        const opt = document.createElement("option");
        opt.value = state.llm.model;
        opt.textContent = state.llm.model;
        modSel.appendChild(opt);
        modSel.value = state.llm.model;
    }
}
...
async function handleGenerateSlidesCurrentWeek() {
  try {
    ...
    if (mode === "llm") {
        if (!state.generatedLLM) { alert("Generate LLM content (Step 5) first."); return; }
        if (!state.llm.model) { alert("Select a Model in Step 5 first."); return; }
    }

    const plan = (mode === "llm") ? (state.generatedLLM || state.generated) : (state.generated || state.generatedLLM);
    if (!plan || !plan.weeks) { alert("Generate a plan first."); return; }

    const status = el("slidesStatus");
    if (status) status.textContent = "Generating slides...";
...
async function handlePdfUpload(file) {
  try {
    ...
    // Prefer augmented_tree (which includes auto-subsections), fall back to toc_tree
    toc_tree: (res.augmented_tree && res.augmented_tree.length) ? res.augmented_tree : res.toc_tree, 
    toc_entries_count: res.augmented_entries_count || res.toc_entries_count 
  };
