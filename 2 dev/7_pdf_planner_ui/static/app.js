...
function renderToc() {
  ...
  // --- NEW HELPER: Icon Button Creator ---
  ...
  const renderNode = (v, depth = 0, parentPath = []) => {
    ...
    right.className = "tocBtnGroup"; // New class for button spacing
    ...
    // 1. View Text
    right.appendChild(
      makeIconBtn(ICONS.view, "view", "Preview Text", () => openPreview(state.activeSourceId, String(node.toc_id)))
    );

    // 2. Edit Text (NEW)
    right.appendChild(
      makeIconBtn(ICONS.edit, "edit", "Edit Content", () => openEditModal(state.activeSourceId, String(node.toc_id), nodeTitle(node)))
    );

    // 3. Assign
    right.appendChild(
      makeIconBtn(ICONS.assign, "assign", "Assign to current week", (ev) => {
    ...
    if (Array.isArray(node.children) && node.children.length) {
      right.appendChild(
        makeIconBtn(ICONS.assignSub, "assign", "Assign entire subtree", () => {
          const wk = String(state.selectedWeek);
          ensureTitlesPathRecursive(node, parentPath);
    ...
    // 5. Unassign/Remove
    right.appendChild(
      makeIconBtn(ICONS.remove, "delete", "Remove from Plan", () => {
