...
# Validate ("snap") bookmark anchors across adjacent pages.
# Some PDFs store outline destinations with a page off-by-one, or the target page is correct
# but the title text appears on the previous / next page due to layout quirks.
ANCHOR_PAGE_VALIDATE = True
ANCHOR_PAGE_VALIDATE_RADIUS = 1          # try page0 +/- this many pages
ANCHOR_PAGE_VALIDATE_MIN_RAW = 0.62      # minimum title match to consider a page a valid anchor
ANCHOR_PAGE_VALIDATE_IMPROVE = 0.05      # require this much improvement to shift away from guessed page

# Secondary fallback: scan forward a few pages for the heading if not found on page0 or adjacent pages.
ANCHOR_PAGE_FORWARD_SCAN = True
ANCHOR_PAGE_FORWARD_MAX_PAGES = 8        # scan up to this many pages forward
ANCHOR_PAGE_FORWARD_MIN_RAW = 0.70       # stricter threshold to reduce false positives
ANCHOR_PAGE_FORWARD_REQUIRE_TOP = True   # require the match to be near the top of the page
ANCHOR_PAGE_FORWARD_TOP_FRAC = 0.45      # top fraction of page allowed for the heading match
...
def _scrub_utf8(s: str) -> str:
    """Scrub surrogate characters that cause JSON encoding crashes."""
    if not s:
        return ""
    # 'replace' will turn bad characters into  instead of crashing
...
def _best_heading_match_on_page(page: fitz.Page, y_hint: float, title: str) -> Tuple[float, float, Optional[float]]:
    """Return (best_effective, best_raw, best_y0) for matches of `title` on `page`.

    Effective score includes a small positional bonus to prefer top-of-page matches.
    This is used to validate ToC anchors across adjacent pages.
    """
    blocks = _block_list(page)
    h = float(page.rect.height)

    # If y_hint is valid, consider a window first; otherwise scan full page.
    use_window = (y_hint is not None) and (0.0 <= float(y_hint) <= h)
    if use_window:
        y0_min = max(0.0, float(y_hint) - TITLE_SNAP_Y_WINDOW_UP)
        y0_max = min(h, float(y_hint) + TITLE_SNAP_Y_WINDOW_DOWN)
    ...
    # Window may be too restrictive: fall back to full-page scan.
    if use_window and len(blks) < 2:
        blks = []
        for (x0, y0, x1, y1, text, *rest) in blocks:
            t = (text or "").strip()
    ...
    def consider_span(i: int, k: int) -> Optional[Tuple[float, float, float]]:
        ...
        cand = " ".join(parts).strip()
        raw = _title_match_score(cand, title)
        pos_bonus = 0.08 * (1.0 - min(1.0, max(0.0, y0 / max(h, 1.0))))
        return raw + pos_bonus, raw, y0
    ...
    return float(best_eff), float(best_raw), (float(best_y) if best_y is not None else None)


def _validate_anchor_page0(doc: fitz.Document, page0: int, y_hint: float, title: str) -> int:
    """Validate that `page0` really contains `title`; otherwise try +/- 1 page.

    This defends against:
      - off-by-one page numbering mismatches in outline destinations
      - TOC entries pointing to a nearby page due to layout / section breaks
    """
    if not ANCHOR_PAGE_VALIDATE:
        return page0
    if page0 < 0 or page0 >= doc.page_count:
        return page0

    cur_page = doc.load_page(page0)
    cur_eff, cur_raw, _ = _best_heading_match_on_page(cur_page, y_hint, title)

    # If it's already a strong match, keep it.
    if cur_raw >= max(TITLE_MATCH_RATIO - 0.02, ANCHOR_PAGE_VALIDATE_MIN_RAW + 0.06):
        return page0
    ...
    for d in range(1, ANCHOR_PAGE_VALIDATE_RADIUS + 1):
        for cand in (page0 - d, page0 + d):
            if cand < 0 or cand >= doc.page_count:
                continue
            p = doc.load_page(cand)
            eff, raw, _ = _best_heading_match_on_page(p, 0.0, title)
            if eff > best_eff:
                best_eff, best_raw, best_page = eff, raw, cand


    # Secondary forward scan: if we still have no plausible match, scan forward a few pages.
    if ANCHOR_PAGE_FORWARD_SCAN and best_raw < ANCHOR_PAGE_VALIDATE_MIN_RAW:
        stop = min(doc.page_count - 1, page0 + ANCHOR_PAGE_FORWARD_MAX_PAGES)
        for cand in range(page0 + 1, stop + 1):
            p = doc.load_page(cand)
            eff, raw, y0 = _best_heading_match_on_page(p, 0.0, title)
            if raw < ANCHOR_PAGE_FORWARD_MIN_RAW:
                continue
            if ANCHOR_PAGE_FORWARD_REQUIRE_TOP and y0 is not None:
                h = float(p.rect.height)
                if y0 > (ANCHOR_PAGE_FORWARD_TOP_FRAC * h):
                    continue
            if eff > best_eff:
                best_eff, best_raw, best_page = eff, raw, cand
            # If we got an excellent match, stop early.
            if raw >= 0.92:
                break

    # Shift only if we found a plausible title match and it is meaningfully better.
    if best_page != page0 and best_raw >= ANCHOR_PAGE_VALIDATE_MIN_RAW:
        if (best_eff >= cur_eff + ANCHOR_PAGE_VALIDATE_IMPROVE) or (cur_raw < (ANCHOR_PAGE_VALIDATE_MIN_RAW - 0.10)):
            return best_page
...
def _score_orientation(doc: fitz.Document, toc_items: List[List[Any]], flip_y: bool) -> int:
    ...
    for row in toc_items[:SCORE_SAMPLE_N]:
        ...
        if not isinstance(dest, dict):
            continue
        dp = int(dest.get("page", (page1 - 1 if isinstance(page1, int) else 0)))
        # Normalize dest page to 0-based: in some PDFs this equals the 1-based `page1`.
        p0 = (dp - 1) if (isinstance(page1, int) and dp == page1) else dp
...
def _build_entries_with_paths_and_anchors(doc: fitz.Document) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], bool]:
    ...
    # --- FIX START: Improved Hierarchy Tracker ---
    active_part_level = -1
    part_shift = 0
    # --- FIX END ---
    ...
    for row in toc_items:
        ...
        # --- FIX START: Recursive Hierarchy Repair ---
        # 1. Check if this is a new "Part" (Reset shift)
        is_part_match = re.match(r"^\s*part\s+[:\.]?\s*(?:\d+|[IVX]+|[A-Z]+)\b", title, re.IGNORECASE)

        if is_part_match:
            active_part_level = lvl
            part_shift = 0  # A new part is never shifted itself
        
        elif active_part_level != -1:
            # 2. Inside a Part scope
            if lvl == active_part_level:
                # Found a Sibling (e.g., Chapter 1 at same level as Part 1)
                # We must indent this item AND all its future children.
                part_shift = 1
                lvl += part_shift
            
            elif lvl > active_part_level:
                # Found a Child (e.g., Section 1.1 under Chapter 1)
                # Apply the current shift to keep it nested under the Chapter
                lvl += part_shift
            
            elif lvl < active_part_level:
                # We popped back up (e.g., Back to a main Section outside Parts)
                active_part_level = -1
                part_shift = 0
        # --- FIX END ---
        ...
        if isinstance(dest, dict) and "page" in dest:
            dp = int(dest["page"])
            # get_toc() returns page1 as 1-based; some PDFs also store dest["page"] as 1-based.
            # Normalize to 0-based, but keep compatibility if dest["page"] is already 0-based.
            page0 = (dp - 1) if dp == page1 else dp
        else:
            page0 = page1 - 1
        ...
        # Extra validation: if the title text is not on the guessed page, try +/-1 page.
        # This book (and others) sometimes has outline targets that are off by one page.
        page0_valid = _validate_anchor_page0(doc, page0, y, title)
        if page0_valid != page0:
            page0 = page0_valid
            page = doc.load_page(page0)
            h = float(page.rect.height)
            y = 0.0
    ...
    entries.sort(key=lambda e: (e["page_0based"], e["toc_id"]))
...
# ============================================================
# Text File Extraction (for "Paste Text" feature)
# ============================================================

def _extract_text_for_ui(
    text_path: Path,
    out_dir: Path,
    source_id: str,
    work_id: str,
    ...
    """
    Handle .txt files by treating the entire content as a single 'node' / section.
    """
    ...
    if not raw_text:
        raw_text = "(Empty text file)"

    # Clean text
    clean_text = _scrub_utf8(raw_text)

    # Use filename as title
    raw_name = text_path.name
    if "__" in raw_name:
        # Split only on the first double underscore
        _, original_filename = raw_name.split("__", 1)
    else:
        original_filename = raw_name
        
    # Create a pretty title from the original filename (remove extension, underscores to spaces)
    ...
    info = {
        ...
        "pdf_path": str(text_path), # field name kept for compatibility
        "page_count": 1,
    }
    ...
    out_txt_path = leaf_dir / "section.txt"
    out_txt_path.write_text(clean_text, encoding="utf-8")

    # Construct single Entry
    entry = {
        **info,
        "doc_kind": "text_file",
        "flip_y_used": False,
        ...
        # Text Metrics
        "text_path_main": str(out_txt_path),
        "text_path": str(out_txt_path),
        "text_chars_main": len(clean_text),
        "text_chars": len(clean_text),
        "text_words_main": len(clean_text.split()),
        "text_preview_main": clean_text[:MAX_PREVIEW_CHARS],
        "text_preview": clean_text[:MAX_PREVIEW_CHARS],

        # Synthetic Geometry (Page 1)
        "start": {"page_0based": 0, "page_1based": 1, "y": 0.0},
        "end": {"page_0based": 0, "page_1based": 1, "y": 1000.0},
        
        "has_children_effective": False,
        "has_toc_children": False,
        "auto_children_count": 0,
        "synthetic": True,
        "synthetic_source": "pasted_text"
    }
    ...
    # Write toc.json
    toc_tree = [ {k: v for k, v in entry.items() if k not in ["text_path_main", "text_preview_main"]} ]
    ...
    toc_payload = {
        ...
        "sections_written": 1,
        "warnings": 0,
        "augmented_tree": toc_tree # Important for UI consistency
    }
...
def extract_pdf_for_ui(
    ...
    if pdf_path.suffix.lower() == ".txt":
        return _extract_text_for_ui(pdf_path, out_dir, source_id, work_id, unit_code)
    ...
    with fitz.open(str(pdf_path)) as doc:
        ...
        for e in entries:
            e["title"] = _scrub_utf8(e["title"])
            e["titles_path"] = [_scrub_utf8(t) for t in e["titles_path"]]
            e["next_peer_title"] = _scrub_utf8(e.get("next_peer_title") or "")

        # Rebuild the tree using the now-clean entries
        toc_tree = _build_tree(entries)
        # --- FIX END ---
        ...
        with open(sections_jsonl_path, "w", encoding="utf-8") as f:
            for e in entries:
                toc_id = int(e["toc_id"])
                title = _scrub_utf8(e["title"])
                titles_path = [_scrub_utf8(t) for t in e["titles_path"]]
                ...
                main_text = _scrub_utf8(main_text)  # <--- Apply scrub here
                # --- FIX END ---
                ...
                full_with_children_txt = _scrub_utf8(full_with_children_txt) # <--- Apply scrub here
                # --- FIX END ---
                ...
                next_peer_title = _scrub_utf8(e.get("next_peer_title") or "").strip()
                ...
                if use_auto_children:
                    for idx, h in enumerate(discovered):
                        ...
                        raw_title = (f"{num} {ttl}".strip() if num else ttl)
                        child_title = _scrub_utf8(raw_title)
                        ...
                        child_text = _scrub_utf8(child_text) # <--- Apply scrub here
                        # --- FIX END ---
        ...
        augmented_entries_sorted = sorted(
            augmented_entries,
            key=lambda e: (e["page_0based"], e["toc_id"])
        )
