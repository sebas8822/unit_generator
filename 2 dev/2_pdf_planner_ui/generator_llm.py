...
class ContentGenerator:
    ...
    def validate_server_and_model(self) -> Dict[str, Any]:
        """
        Connects to Ollama to verify:
        1. The server is reachable (list models).
        2. The requested model exists in the list.
        """
        try:
            # 1. Try to list models to check server connectivity
            # If the server is down, this will raise an Exception immediately
            response = self.client.list()
            
            # 2. Get list of model names
            # Standard Ollama API returns 'models' list
            models = response.get('models', [])
            
            # Extract names, falling back to 'model' key if 'name' is missing
            available_names = [m.get('name', m.get('model', '')) for m in models]
            
            # --- DEBUGGING OUTPUT ---
            print("--------------------------------------------------")
            print(f"DEBUG: Requested Model: '{self.model}'")
            print(f"DEBUG: Available Models: {available_names}") # Commented out to reduce noise if many models
            print("--------------------------------------------------")

            clean_requested = self.model.strip()
            found = False

            for m_name in available_names:
                # 1. Exact match (e.g. "llama3:latest" == "llama3:latest")
                if m_name == clean_requested:
                    found = True
                    break
                
                # 2. Flexible match for tagless requests
                # If user asks for "llama3", match "llama3:latest" or "llama3:8b"
                if ":" not in clean_requested:
                    if m_name.startswith(clean_requested + ":"):
                        found = True
                        break
            ...
            if not found:
                return {
                    "ok": False, 
                    "message": f"Server is online, but model '{clean_requested}' was not found."
                }
                
            return {"ok": True, "message": f"Ollama online. Model '{clean_requested}' found."}

        except Exception as e:
            print(f"DEBUG: Validation Error: {e}")
            # Return the actual error message (e.g., Connection Refused)
            return {
                "ok": False, 
                "message": f"Could not connect to Ollama. Verify it is running on port 11434. Error: {str(e)}"
            }
