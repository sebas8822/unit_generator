...
def _bin_pack(items: List[Dict[str, Any]], k: int) -> List[List[Dict[str, Any]]]:
    """
    Greedy bin packing (LPT-style): sort by weight desc, always add to lightest bin.
    Each 'item' must have a numeric 'weight'.
    """
    k = max(1, int(k))
    ...
    for it in sorted(items, key=lambda x: x.get("weight", 0), reverse=True):
        ...
        bins[j].append(it)
        weights[j] += int(it.get("weight", 0) or 0)
    return bins
...
def generate_plan_from_ui_selection(
    ...
    for w in range(1, weeks_count + 1):
        ...
        decks = _bin_pack(partition_units, sessions_per_week)
