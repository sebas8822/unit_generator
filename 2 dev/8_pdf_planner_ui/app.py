...
@app.post("/api/project/{course_id}/section_text/{source_id}/{toc_id}")
async def update_section_text(course_id: str, source_id: str, toc_id: str, payload: Dict[str, Any]):
    """
    Updates the text content of a specific section.
    """
    new_text = payload.get("text", "")
    
    # 1. Load index to find the file path
    ...
    if not text_path:
        raise HTTPException(status_code=400, detail="No text file associated with this section.")

    # 2. Security Check (Path Traversal)
    p = Path(text_path)
    try:
        rp = p.resolve()
        # Ensure the file is actually inside the project's extracted folder
        base_extracted = (_project_paths(course_id)["extracted"]).resolve()
        if not str(rp).startswith(str(base_extracted)):
    ...
    return JSONResponse({"ok": True, "size": len(new_text)})
    ...
    if paths["extracted"].exists():
        for src_dir in paths["extracted"].iterdir():
            ...
            sources[source_id] = {
                ...
                # --- FIX: Send Deep Structure to Frontend ---
                "augmented_tree": toc_obj.get("augmented_tree", []),
                "augmented_entries_count": toc_obj.get("augmented_entries_count", 0),
                "doc_kind": toc_obj.get("doc_kind", "publication")
            }
...
@app.delete("/api/project/{course_id}")
def delete_project(course_id: str):
    cid = _require_safe_course_id(course_id)
    pdir = _project_dir(cid)
...
async def upload_pdf(course_id: str, file: UploadFile = File(...)):
    ...
    # --- FIX: Allow .txt files for the Paste Text feature ---
    ...
    # The extractor needs the correct extension to decide how to process it
    file_path = paths["uploads"] / f"{source_id}__{Path(file.filename).name}"
    ...
    result = extract_pdf_for_ui(
        pdf_path=file_path,  # This path now ends in .pdf OR .txt
        out_dir=out_dir,
        source_id=source_id,
        work_id="local_upload",
        unit_code=course_id,
    )
    result["source_file"] = Path(file.filename).name
...
@app.delete("/api/project/{course_id}/source/{source_id}")
def delete_source(course_id: str, source_id: str):
    paths = _ensure_project(course_id)
    
    # 1. Remove extracted folder
    extracted_dir = paths["extracted"] / source_id
    if extracted_dir.exists():
        shutil.rmtree(extracted_dir)
        
    # 2. Remove original upload file
    # Uploads are named like "{source_id}__{filename}"
    found = False
    for f in paths["uploads"].glob(f"{source_id}__*"):
        try:
            f.unlink()
            found = True
        except Exception:
    ...
    # 3. Cleanup from Plan (Optional but recommended)
    # We load the plan, remove items referencing this source, and save.
    plan = _read_json_or_default(paths["plan_json"], {})
    weeks = plan.get("weeks", {})
    changed = False
    for w_key, w_data in weeks.items():
        items = w_data.get("items", [])
        original_len = len(items)
        # Filter out items belonging to this source
        w_data["items"] = [it for it in items if str(it.get("source_id")) != source_id]
        if len(w_data["items"]) != original_len:
            changed = True
            
    if changed:
        paths["plan_json"].write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding="utf-8")

    return JSONResponse({"ok": True, "deleted_extracted": True, "deleted_upload": found, "plan_updated": changed})
