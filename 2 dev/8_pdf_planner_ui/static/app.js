...
// --- SVG ICONS ---
const ICONS = {
  view: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>`,
  ...
  assign: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>`,
  assignSub: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M11 19H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2v2"></path><line x1="12" y1="21" x2="12" y2="13"></line><line x1="9" y1="17" x2="15" y2="17"></line></svg>`, // Folder-ish plus
  remove: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`,
  general: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>`,
  generalAll: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path><path d="M12 22v-4"></path><path d="M12 6V2"></path><path d="M4 12h4"></path><path d="M20 12h-4"></path></svg>`
};
...
function assignNodeToWeekAsGeneral(sourceId, node, weekStr) {
  // Prevent duplicates in the same week
  if (!state.plan.weeks[weekStr]) state.plan.weeks[weekStr] = { topic: "", items: [] };
  const w = state.plan.weeks[weekStr];
...
// ADD LOGIC TO ASSIGN TO ALL WEEKS
function assignNodeToAllWeeksAsGeneral(sourceId, node) {
    const totalWeeks = state.weeksCount || 12;
    for (let i = 1; i <= totalWeeks; i++) {
        assignNodeToWeekAsGeneral(sourceId, node, String(i));
    }
}
...
function renderPdfList() {
  ...
  for (const [sourceId, src] of entries) {
    ...
    // --- FIX: Override the default 'center' alignment from .listItem class ---
    ...
    titleText.style.textAlign = "left"; // Explicitly ensure left text alignment
    
    // CSS Truncation (2 lines)
...
function renderToc() {
  ...
  // --- NEW LOGIC: Map toc_id -> Array of Weeks ---
  const assignmentsByTocId = new Map(); // toc_id -> { weeks: Set<int>, isGeneral: boolean }
  ...
  for (const [wk, wobj] of Object.entries(state.plan?.weeks || {})) {
    ...
    for (const it of (wobj?.items || [])) {
      if (String(it.source_id) === String(state.activeSourceId)) {
        const tid = String(it.toc_id);
        if (!assignmentsByTocId.has(tid)) {
            assignmentsByTocId.set(tid, { weeks: new Set(), isGeneral: false });
        }
        const rec = assignmentsByTocId.get(tid);
        rec.weeks.add(weekNum);
        if (it.type === "general") rec.isGeneral = true;
      }
    }
  }
  ...
  const matchesFilters = (n) => {
    if (state.tocAssignedOnly && !assignmentsByTocId.has(String(n.toc_id))) return false;
  ...
  const makeIconBtn = (iconSvg, cls, title, onClick) => {
    const b = document.createElement("button");
    b.className = `tocActionBtn ${cls}`;
    b.innerHTML = iconSvg;
    b.title = title;
    b.onclick = (e) => {
      e.preventDefault();
  ...
  const renderNode = (v, depth = 0, parentPath = []) => {
    ...
    // --- ASSIGNMENT BADGE LOGIC (Handle Multiple Weeks) ---
    const assignData = assignmentsByTocId.get(String(node.toc_id));
    
    if (assignData && assignData.weeks.size > 0) {
      const weeksArr = Array.from(assignData.weeks).sort((a, b) => a - b);
      const isAll = (weeksArr.length === (state.weeksCount || 12)); // Check if assigned to all
      
      const tag = document.createElement("div");
      tag.className = "tagWeek";
      
      const genLabel = assignData.isGeneral ? " (Gen)" : "";
      
      if (isAll) {
          tag.textContent = `All Weeks${genLabel}`;
          tag.style.setProperty("--hue", 280); // Purple for All
      } else if (weeksArr.length > 3) {
           tag.textContent = `${weeksArr.length} Weeks${genLabel}`; // Condensed
           tag.title = "Weeks: " + weeksArr.join(", ");
           tag.style.setProperty("--hue", 200);
      } else {
           tag.textContent = `Wk ${weeksArr.join(", ")}${genLabel}`;
           // Use hue of first week
           const hue = (weeksArr[0] * 45) % 360;
           tag.style.setProperty("--hue", hue);
      }

      // If purely General, force Orange hue
      if (assignData.isGeneral) {
           // tag.style.setProperty("--hue", 30); // Orange
           // Or keep week colors but show text. Let's stick to week colors for organization
      }
    ...
    // 1. View
    right.appendChild(makeIconBtn(ICONS.view, "view", "Preview Text", () => openPreview(state.activeSourceId, String(node.toc_id))));

    // 2. Edit
    right.appendChild(makeIconBtn(ICONS.edit, "edit", "Edit Content", () => openEditModal(state.activeSourceId, String(node.toc_id), nodeTitle(node))));

    // 3. Assign Standard
    right.appendChild(makeIconBtn(ICONS.assign, "assign", "Assign to current week", () => {
        ensureTitlesPathRecursive(node, parentPath);
        assignNodeToWeek(state.activeSourceId, node, String(state.selectedWeek));
        renderAll();
    }));
    
    // 4. Assign General (Single Week)
    right.appendChild(makeIconBtn(ICONS.general, "general", "Assign General (Current Week)", () => {
        ensureTitlesPathRecursive(node, parentPath);
        assignNodeToWeekAsGeneral(state.activeSourceId, node, String(state.selectedWeek));
        renderAll();
    }));

    // 5. NEW: Assign General (ALL WEEKS)
    right.appendChild(makeIconBtn(ICONS.generalAll, "generalAll", "Assign General (ALL Weeks)", () => {
        if(!confirm("Assign this node as General Content to ALL weeks?")) return;
        ensureTitlesPathRecursive(node, parentPath);
        assignNodeToAllWeeksAsGeneral(state.activeSourceId, node);
        renderAll();
    }));

    // 6. Assign Subtree
    if (Array.isArray(node.children) && node.children.length) {
      right.appendChild(makeIconBtn(ICONS.assignSub, "assign", "Assign entire subtree", () => {
          ensureTitlesPathRecursive(node, parentPath);
          assignSubtreeToWeek(state.activeSourceId, node, String(state.selectedWeek));
          renderAll();
      }));
    }

    // 7. Remove
    right.appendChild(makeIconBtn(ICONS.remove, "delete", "Remove from Plan", () => {
...
function renderWeekTreeNode(node, depth, opts = {}) {
  ...
  const makeLine = () => {
    ...
    const isGeneral = sortedSelected.some(item => item && item.type === "general");
    const showGenBadge = (opts && opts.showGenBadge !== false);

    const label = document.createElement("div");
    label.className = "wkLabel";
    if (isGeneral && showGenBadge) {
      label.innerHTML = `<span class="badge general">GEN</span> ${escapeHtml(node.label)}`;
    } else {
  ...
  if (hasKids) {
    ...
    for (const ch of children) kidsWrap.appendChild(renderWeekTreeNode(ch, depth + 1, opts));
...
function renderWeekSummary() {
  ...
  const makeSectionHeader = (title, count) => {
    const head = document.createElement("div");
    head.className = "wkSectionTitle";

    const left = document.createElement("div");
    left.className = "wkSectionLabel";
    left.textContent = title;

    const right = document.createElement("div");
    right.className = "wkSectionMeta muted small";
  ...
  for (let i = 1; i <= state.weeksCount; i++) {
    ...
    const normalItems = items.filter(it => it && it.type !== "general");
    const generalItems = items.filter(it => it && it.type === "general");
    ...
    const parts = [];
    if (normalItems.length) parts.push(`${normalItems.length} sections`);
    if (generalItems.length) parts.push(`${generalItems.length} general`);
    const detail = parts.length ? ` • ${parts.join(" • ")}` : "";
    sum.innerHTML = `<div><b>Week ${i}</b> <span class="muted small">(${items.length} items${detail})</span></div>`;
    ...
    if (!items.length) {
      body.innerHTML = `<div class="muted small">No sections assigned.</div>`;
    } else {
      // 1) Sections (non-general)
      if (normalItems.length) {
        body.appendChild(makeSectionHeader("Sections", normalItems.length));
        const tree = buildWeekTree(normalItems);
        const sources = Object.values(tree.children).sort((a, b) => a.minOrderIndex - b.minOrderIndex);
        for (const srcNode of sources) body.appendChild(renderWeekTreeNode(srcNode, 0, { showGenBadge: true }));
      }

      // Divider between sections and general
      if (normalItems.length && generalItems.length) {
      ...
      if (generalItems.length) {
        body.appendChild(makeSectionHeader("General", generalItems.length));
        const treeG = buildWeekTree(generalItems);
        const sourcesG = Object.values(treeG.children).sort((a, b) => a.minOrderIndex - b.minOrderIndex);
        for (const srcNode of sourcesG) body.appendChild(renderWeekTreeNode(srcNode, 0, { showGenBadge: false }));
      }
...
function renderSlideGenerationCard() {
  ...
  if (sourceLabel) {
      if (!hasModel) {
          // Keep the red warning, but make it fit the dark theme better or just use text
          sourceLabel.innerHTML = `<span style="color:var(--danger); font-weight:bold;">⚠️ Select Model in Step 5</span> <span class="muted small">(required)</span>`;
          ...
          sourceLabel.innerHTML = `Output: <code>final_presentations/${folder}/</code>`;
          sourceLabel.style.color = ""; // Clear any previous inline styles
          // --------------------
...
let currentEditTarget = { sourceId: null, tocId: null };

async function openEditModal(sourceId, tocId, title) {
    const modal = document.getElementById("editModal");
    const titleEl = document.getElementById("editTitle");
    const contentEl = document.getElementById("editContentInput");
    
    currentEditTarget = { sourceId, tocId };
    titleEl.textContent = `Edit: ${title}`;
    contentEl.value = "Loading...";
    modal.classList.remove("hidden");

    try {
        const data = await apiGet(apiUrl(`/section_text/${encodeURIComponent(sourceId)}/${encodeURIComponent(tocId)}?max_chars=50000`));
        contentEl.value = data.text || "";
    } catch(e) {
        contentEl.value = "Error loading text: " + e.message;
    }
}

async function saveCurrentEdit() {
    if (!currentEditTarget.sourceId) return;
    ...
    try {
        await apiPost(apiUrl(`/section_text/${encodeURIComponent(currentEditTarget.sourceId)}/${encodeURIComponent(currentEditTarget.tocId)}`), {
            text: content
        });
        
        document.getElementById("editModal").classList.add("hidden");
        // Optional: Flash success message or refresh ToC Preview if open
    } catch(e) {
...
async function handlePdfUpload(file) {
  ...
  try {
    ...
    state.sources[res.source_id] = { 
        ...
        toc_entries_count: (useAugmented && res.augmented_entries_count) 
                           ? res.augmented_entries_count 
                           : res.toc_entries_count,
        doc_kind: res.doc_kind
    };
...
async function boot() {
  ...
  bindClick("saveEditBtn", saveCurrentEdit);
  bindClick("closeEditBtn", () => document.getElementById("editModal").classList.add("hidden"));
